  ...
  <div class="main">
    <van-swipe :autoplay="3000">
      <van-swipe-item v-for="(image, index) in images" :key="index">
        <img style="height: 200px;width: 100%" v-lazy="image" />
      </van-swipe-item>
    </van-swipe>
    <van-grid v-for="(item,index) in setMeals" :key="index" :border="true" :column-num="2">
      ...
      <van-grid-item v-if="index%2===0">
        <div v-if="index<setMeals.length-1">
          <van-image @click="intoSetMeal(setMeals[index+1])" style="height: 200px;width: 100%" :src="getIndexImage(setMeals[index+1])"/>
          <div>
            <span >{{setMeals[index+1].sname}}</span>
            <div style="color: red">
              <span style="float: left;display:inline-block;height:20px;font-size:20px;">￥{{setMeals[index+1].price}}</span>
...
export default {
  ...
  created() {
    ...
    this.$api.get("setMeal/get/all").then(res => {
      ...
      console.log(res)
      this.setMeals = res;
      this.$store.commit("setSetMealsList", this.setMeals)
    }).catch(reason => {
      console.log(reason)
    });
  },
  mounted() {
    ...
    this.$store.commit("setRouterPathNew","/");
